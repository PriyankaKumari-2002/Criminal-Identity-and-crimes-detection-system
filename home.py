...
def basicPageSetup(pageNo):
    ...
    back_button = tk.Button(pages[pageNo], image=back_img, bg="#F88379", bd=0, highlightthickness=0,
           activebackground="#F88379", command=goBack)
    back_button.image = back_img
    back_button.place(x=10, y=10)

    heading = tk.Label(pages[pageNo], fg="white", bg="#F88379", font="Arial 20 bold", pady=10)
    heading.pack()

    content = tk.Frame(pages[pageNo], bg="#F88379", pady=20)
    content.pack(expand="true", fill="both")

    left_frame = tk.Frame(content, bg="#F88379")
    left_frame.grid(row=0, column=0, sticky="nsew")

    right_frame = tk.LabelFrame(content, text="Detected Criminals", bg="#F88379", font="Arial 20 bold", bd=4,
                             foreground="#FFFF00", labelanchor="n")
...
def showImage(frame, img_size):
    ...
    if (img_label == None):
        img_label = tk.Label(left_frame, image=img, bg="#F88379")
        img_label.image = img
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if(len(path_list) < 5):
        ...
        prev_slide = tk.Button(slide_control_panel, image=back_img, bg="#F88379", bd=0, highlightthickness=0,
                            activebackground="#F88379", command=lambda : getNewSlide("prev"))
        ...
        next_slide = tk.Button(slide_control_panel, image=next_img, bg="#F88379", bd=0, highlightthickness=0,
                            activebackground="#F88379", command=lambda : getNewSlide("next"))
...
def getPage1():
    ...
    btn_grid = tk.Frame(left_frame, bg="#F88379")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#2196f3",
           fg="#FFFF00", pady=10, bd=0, highlightthickness=0, activebackground="#FFFF00",
           activeforeground="black").grid(row=0, column=0, padx=25, pady=25)


    # Creating Scrollable Frame
    canvas = tk.Canvas(right_frame, bg="#F88379", highlightthickness=0)
    canvas.pack(side="left", fill="both", expand="true", padx=30)
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="#F88379", bd=0,
                          activebackground="#00bcd4", bg="#FFFF00", relief="raised")
    ...
    for i, field in enumerate(input_fields):
        ...
        if(i != ip_len-1):
            ...
            menu.configure(font="Arial 13", bg="white", activebackground="#E1AD01", bd=0)

    tk.Button(scroll_frame, text="Register", command=lambda: register(entries, required, menu_var), font="Arial 15 bold",
           bg="#2196f3", fg="white", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#E1AD01",
...
def getPage2():
    ...
    tk.Button(btn_grid, text="Select Image", command=selectImage, font="Arial 15 bold", padx=20, bg="#2196f3",
            fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#E1AD01",
...
btn_frame = tk.Frame(pages[0], bg="#2196f3", pady=30)
btn_frame.pack()
...
for btn in btn_frame.winfo_children():
    btn.configure(font="Times", width=17, bg="#00FFFF", fg="black",
